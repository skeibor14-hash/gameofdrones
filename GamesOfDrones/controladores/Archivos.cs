	...
	public class Archivos{
		...
		public static void main( WebApplication app ){
			app.MapGet( _sistema_.Lector_ENV.obtener("archivo_angular"),( HttpContext context ) => {
				return Archivos.responder_cabezera( context, "application/javascript", "inline; filename=angular.min.js", true, false, false, "angular.min.js" );
			});
			app.MapGet( _sistema_.Lector_ENV.obtener("archivo_favicon"),( HttpContext context ) => {
				return Archivos.responder_imagen( context, "image/ico", "inline; filename=favicon.ico", true, false, false, "favicon.ico" );
			});
			app.MapGet( _sistema_.Lector_ENV.obtener("archivo_scripts"),( HttpContext context ) => {
				return Archivos.responder_cabezera( context, "application/javascript", "inline; filename=scripts.js", true, false, false, "scripts.js" );
			});
			app.MapGet( _sistema_.Lector_ENV.obtener("archivo_estilos"),( HttpContext context ) => {
				return Archivos.responder_cabezera( context, "text/css", "inline; filename=estilos.css", true, false, false, "estilos.css" );
			});
		}

		/*responder_cabezera*/
		public static Microsoft.AspNetCore.Http.IResult responder_imagen( HttpContext context, string mime, string disposicion, bool cache, bool robots, bool csp, string archivo ){
			/**/
			/*Cabezeras*/
				Archivos.cabezeras_constantes( context );
				Archivos.cabezeras_cache( context, cache );
				Archivos.cabezeras_robots( context, robots );
				Archivos.cabezeras_csp( context, csp );
			/**/
			/*Obtener Archivo*/
				...
				if( !File.Exists( ruta_archivo ) ){
					...
					/*Crear respuesta*/
						text += "{";
							...
							text += @",""mensaje"":"	+ @"""Archivo no encontrado.""";
							text += @",""extras"":"		+ "{}";
						text += "}";
					/**/
					/*Enviar error*/
			...
			/*Elegir metodo*/
				if(
					...
					/*Aplica Datos*/
						...
						header += ", report-to"							+ "'none'";																							/*URL para reportar alertas de CSP*/
						header += ", require-trusted-types-for"			+ "'script'";																						/*Exige uso de elementos sanitizados para injecciones HTML en JS*/
						header += ", trusted-types"						+ "'none'";																							/*Listado de politicas que sanitizan HTML injectado en JS*/
						/*header += ", upgrade-insecure-requests";*/																										/*Convierte todas las URL de http a https*/
						header += ", block-all-mixed-content";																												/*Evita uso de http en https*/
						header += ", report-uri"						+ "'none'";																							/*URL para reportar alertas de CSP (repor-to posee predecedencia)*/
					/**/
				}
				else{
