...
namespace _sistema_{
	...
	public static class Modelos{
		...
		/*Codigo de manipulacion de tablas*/
		public static string Creacion			= "CREATE TABLE IF NOT EXISTS [TABLE]( [ESTRUCTURA] );";		/*Solo se ejecuta si la tabla no existe*/
		public static string Insersion_inicio	= "INSERT INTO [TABLE]( [CREADO_COL],[COLS_1] )VALUES";			/*Insersion estandar inicial*/
		public static string Insersion_fin		= ",( [CREADO_VAL],[COLS_2] )";								/*N Objetos insertables*/
		public static string Actualizacion		= "UPDATE [TABLE] SET [ACTUALIZADO_COL],[COLS_1] WHERE [ID]";	/*Actualizacion estandar; Solo se actualizará por ID*/
		public static string Busqueda			= "SELECT [COLS_1] [FROM] [WHERE] [OFFSET] [LIMIT] [ORDER]";	/*Selección estandar; Solo se utilizara estos queries*/

		/*Unic actual*/
		static public string tiempo_unix(){
			...
			/*Total fecha*/
			long unix_timestamp	= Convert.ToInt64( (UTC - EPOCH).TotalSeconds );

			/*String UTC para DB*/
			string unix_timestamp_string = unix_timestamp.ToString();

			/*FIN*/
			return unix_timestamp_string;
		}
		...
		*/
		static public void verificar_tabla( string debug, string nombre_tabla, string estructura_tabla, string campos_insertables, string campo_creado, string datos_migrables ){
		...
		*/
		...
		static public void insertar_item( string debug, string nombre_tabla, string campos_insertables, string campo_creado, string datos_insertables ){
			...
			/**/
			/*Remplazo de creado VAL*/
				insersion_fin = insersion_fin.Replace("[CREADO_VAL]",			_sistema_.Modelos.tiempo_unix()				);
			/**/
			/*Union de elementos*/
		...
		public static string opciones( string debug, string nombre_tabla ){
			...
			/**/
			/*Iterar resultado*/
				foreach( string filas in _sistema_.Modelos.seleccionar_tabla( debug, nombre_tabla, "id,nombre" ).Split("$") ){
